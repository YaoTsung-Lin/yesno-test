...
let stage='start',current=0,title='YES / NO 投票';
let results=questions.map(()=>({yes:0,no:0})); let votes={};
function state(){return {stage,current,title,questions,results};}
io.on('connection',s=>{
 s.emit('state',state());
 s.on('setTitle',t=>{title=t||'YES / NO 投票';io.emit('state',state());});
 s.on('start',()=>{stage='quiz';current=0;io.emit('state',state());});
 s.on('finish',()=>{stage='end';io.emit('state',state());});
 ...
 s.on('next',()=>{if(current<questions.length-1){current++;io.emit('state',state());}});
